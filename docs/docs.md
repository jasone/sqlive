...
By looking at the schema, we can see that the two tables are related through the columns `c_custkey` and `o_custkey`.
...
In this example, we created a virtual view called `positive_bal_customers` that regroups all the customers with a positive balance.
...
With this command we instructed SQLive to send all the changes relative to the virtual view `negative_balance" to the file `/tmp/negative_balance`.
...
We just declared an ephemeral data stream called `customer_connect_log`.
...
In this case, we could for example get 10 processes writing on the stream `customer_connect_log` at the same time:
